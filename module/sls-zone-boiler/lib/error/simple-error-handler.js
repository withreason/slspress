...
const UnprocessableEntity = require('./unprocessable-entity-error');
...
class SimpleErrorHandler extends ErrorHandler {
  ...
  handle(error, event, context, callback) {
    ...
    if (error instanceof UnprocessableEntity) {
      return this._responseFactory.unprocessableEntity(this._createErrorBody(error, event, context)).send(callback);
    }
